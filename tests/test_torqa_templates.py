"""examples/torqa/templates/**/*.tq must parse, pass diagnostics, and materialize (CI guard)."""
...
def test_all_torqa_templates_parse_validate_and_materialize(tmp_path):
    root = REPO / "examples" / "torqa" / "templates"
    tq_files = sorted(root.rglob("*.tq"))
    assert tq_files, "expected at least one .tq under examples/torqa/templates"
