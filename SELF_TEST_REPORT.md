...
**Environment:** Windows, `python -m src.torqa_cli` from repo root (after `pip install -e ".[dev]"`).  
...
2. Mutlu yol: aşağıdaki `torqa` veya `python -m src.torqa_cli` komutlarını çalıştır.
...
```bash
# Mutlu yol
python -m src.torqa_cli validate examples/approval_flow.tq
python -m src.torqa_cli validate examples/ai_generated.json
python -m src.torqa_cli doctor examples/approval_flow.tq --profile default
python -m src.torqa_cli doctor examples/approval_flow.tq --profile strict

# Profiller (aynı JSON)
python -m src.torqa_cli validate examples/ai_generated.json --profile default
python -m src.torqa_cli validate examples/ai_generated.json --profile strict
python -m src.torqa_cli validate examples/ai_generated.json --profile review-heavy

# Profiller (yüksek severity — strict FAIL)
python -m src.torqa_cli validate examples/approval_flow.tq --profile default
python -m src.torqa_cli validate examples/approval_flow.tq --profile strict
python -m src.torqa_cli validate examples/approval_flow.tq --profile review-heavy

# Kırık örnekler
python -m src.torqa_cli validate examples/self_test_broken/missing_owner.tq
python -m src.torqa_cli validate examples/self_test_broken/missing_severity.tq
python -m src.torqa_cli validate examples/self_test_broken/unknown_step.tq
```

`torqa` PATH’teyse `python -m src.torqa_cli` yerine `torqa` kullan.
