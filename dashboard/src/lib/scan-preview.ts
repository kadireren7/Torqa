/**
 * Client-only preview heuristics for the dashboard /scan demo.
 * Not the Torqa Python engine — deterministic, browser-side rules.
 */

export type PreviewSeverity = "info" | "review" | "high";
...
export type PreviewDecision = "PASS" | "NEEDS REVIEW" | "FAIL";

export type PreviewSource = "generic" | "n8n";
...
function riskScoreFromFindings(findings: PreviewFinding[]): number {
  let score = 100;
...
function decisionFrom(findings: PreviewFinding[]): PreviewDecision {
  if (findings.some((f) => f.severity === "high")) return "FAIL";
  if (findings.some((f) => f.severity === "review")) return "NEEDS REVIEW";
  return "PASS";
}

function analyzeN8n(doc: Record<string, unknown>, active: boolean | null): PreviewFinding[] {
  ...
  out.sort((a, b) => {
    const order = (s: PreviewSeverity) => (s === "high" ? 0 : s === "review" ? 1 : 2);
...
function analyzeGeneric(data: unknown): PreviewFinding[] {
  const out: PreviewFinding[] = [];
  ...
  if (typeof root.ir_goal === "object" && root.ir_goal !== null) {
    ...
    out.push({
      ...
      target: "json",
      explanation: "Generic JSON preview has limited heuristics in the browser demo.",
  ...
  if (source === "n8n") {
    ...
    if (looksN8n) {
      findings.push({
        ...
        suggested_fix: "Select n8n as the source for richer workflow preview rules.",
      });
    }
  }

  const risk = riskScoreFromFindings(findings);
