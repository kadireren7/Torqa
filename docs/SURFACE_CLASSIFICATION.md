...
|-------|---------|------------------------|
| **TORQA-primary** | Author- and product-facing: surface language, IR contract, examples users copy; primary loop **`torqa build`**; **official product website** source | `examples/torqa/`, `examples/torqa/templates/`, `examples/workspace_minimal/*.tq`, `examples/torqa_self/*.tq` (policy **sources**), `spec/IR_BUNDLE.schema.json`, `docs/TQ_*`, `docs/QUICKSTART.md`, `docs/TRY_TORQA.md`, [`website/`](../website/) (marketing site → `website/dist/site/`), VS Code extension under `editors/vscode-torqa/` |
