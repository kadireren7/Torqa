...
- **CLI** `project-x` or `python -m src.cli.main`: `validate`, `diagnostics`, `explain`, `quality`, `strategy`, `project`, `run`, `ai-suggest`, `patch`, `preview-patch`, `check` (+ global `--json`).
...
- **Rust bridge timeout**: `PROJECT_X_RUST_TIMEOUT_SEC` (default 120s).
