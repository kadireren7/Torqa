...
def _build_html(
    *,
    ...
    n = len(rows)
    top_blocked = [r for r in rows if r.decision == DECISION_BLOCKED][:5]
    top_review = [r for r in rows if r.decision == DECISION_REVIEW][:5]
    ...
    blocked_list = (
        "<ul>" + "".join(f"<li><code>{esc(r.rel_path)}</code> — {esc(r.reason)}</li>" for r in top_blocked) + "</ul>"
        if top_blocked
        else "<p>No blocked files in this report.</p>"
    )
    review_list = (
        "<ul>" + "".join(f"<li><code>{esc(r.rel_path)}</code> — {esc(r.reason)}</li>" for r in top_review) + "</ul>"
        if top_review
        else "<p>No review-required files in this report.</p>"
    )
...
<style>
  ...
  .summary strong.badge {{ display: inline-block; margin-right: 0.5rem; padding: 0.1rem 0.4rem; border-radius: 4px; background: #efefef; }}
  .grid {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 0.75rem; margin-top: 1rem; }}
  .panel {{ border: 1px solid #ddd; border-radius: 4px; padding: 0.6rem 0.7rem; background: #fff; }}
...
{tbody}
  ...
  <div class="summary">
    ...
    <strong class="badge">Total: {n}</strong>
    <strong class="badge">Safe: {safe}</strong>
    <strong class="badge">Needs review: {needs}</strong>
    <strong class="badge">Blocked: {blocked}</strong>
    <p>Recommended next step: {"Fix blocked specs first, then rerun report." if blocked > 0 else "Attach this report to PR/release notes for visibility."}</p>
  </div>
  <div class="grid">
    <section class="panel">
      <h3>Key blocked findings</h3>
      {blocked_list}
    </section>
    <section class="panel">
      <h3>Key review findings</h3>
      {review_list}
    </section>
  </div>
...
def _build_markdown(
    *,
    ...
    parts.append(
        f"- **Recommended next step:** {'Fix blocked specs first, then rerun.' if blocked_n > 0 else 'Share this report in PR/Slack for review traceability.'}\n"
    )
...
def _build_json_artifact(
    *,
    ...
    doc = {
        ...
        "key_findings": {
            "blocked": [{"file": r.rel_path, "reason": r.reason} for r in rows if r.decision == DECISION_BLOCKED][:5],
            "needs_review": [{"file": r.rel_path, "reason": r.reason} for r in rows if r.decision == DECISION_REVIEW][:5],
        },
        "next_steps": [
            "Fix blocked specs first, then rerun report." if blocked > 0 else "No blocked specs under this profile.",
            "Use torqa check <file> for detailed remediation on one artifact.",
            "Attach this artifact in PR/Slack/release notes.",
        ],
        "rows": [
            {
                "file": r.rel_path,
...
def cmd_report(args: object) -> int:
    ...
    if fmt == "html":
        ...
        doc = _build_json_artifact(scope_label=scope, profile=profile, generated_at=gen, rows=rows)
        default_name = "torqa-report.json"
